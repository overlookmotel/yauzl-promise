...
function emit(event, value) {
	...
	if (this._reading) {
		...
		// Event emitted when no action in progress
		// (possibly reader unexpectedly closes or errors).
		// Store error to be returned on next call to
		// `.readEntry()`, `.close()` or `.openReadStream()`.
		if (event != 'error') value = new Error(`Unexpected event '${event}' emitted`);
		this._error = value;
	}
}

function rejectWithStoredError(zipFile, reject) {
	reject(zipFile._error);
	zipFile._error = undefined;
}
