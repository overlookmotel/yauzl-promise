...
class Zip {
	 ...
	 *
	 * @function
	 * @returns {boolean} - `true` if entry is symlink. Otherwise, false.
	 */
	async isSymlink() {
		const fileMode = this._modeFromEntry();
		return ((fileMode & 0o170000) === 0o120000);
	}

	/**
	 * Get file mode from `Entry`. Reference implementation resides [here](https://github.com/fpsqdb/zip-lib/blob/ac447d269218d396e05cd7072d0e9cd82b5ec52c/src/unzip.ts#L380).
	 *
	 * @async
	 * @function
	 * @param {Entry} - - yauzl entry
	 * @returns {number} - file mode
	 */
	_modeFromEntry() {
		const entry = this._readEntryAt(this.entryCount);
	 ...
	 * @async
	 * @returns {Promise<Entry|null>} - `Entry` object for next entry, or `null` if none remaining
	 */
	async _readEntry() {
	 ...
	 * @returns {Promise<Object|null>} - Entry properties or `null` if no Central Directory File Header found
	 */
